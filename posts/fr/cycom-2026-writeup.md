...
```mermaid
flowchart TD
    A[firmware.cpio.gz] -->|cpio + XOR BLACKBOX| B[config en clair]
    B -->|AUTH_LEGACY = nonce ^ 0xc0deface| C[session authentifiée]
    C -->|overflow REPORT → launch_maint_shell| D[shell service]
    D -->|BB_PLUGIN_PATH + .so constructor| E[shell admin]
    E -->|ROR3 + XOR clé + compteur| F[secrets de l'incident]
```
